...
{	
	...
	public function login($username, $password, $verifyCode = '', $isRemember = false, $type = false)
	{
        ...
        $updateData['login_count']		=	array('exp','login_count+1');
        $updateData['last_login_ip']	=	get_client_ip();
        $this->update($updateData);//更新登录信息

        storage_user_action($userInfo['id'],$userInfo['username'],config('BACKEND_USER'),'登录了后台系统');//更新行为
